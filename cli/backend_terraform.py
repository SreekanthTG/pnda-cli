...
class TerraformBackend(BaseBackend):
    ...
    def fill_instance_map(self):
        ...
        return_code, stdout, stderr = self._terraform.output("-json")
        print stdout
        instance_map = {}
        return instance_map
    ...
    def pre_install_pnda(self, node_counts):
        ...
        return_code, stdout, stderr = self._terraform.apply(capture_output=False)
        if return_code != 0:
    ...
    def pre_destroy_pnda(self):
        ...
        self._set_up_terraform_resources(None)

        CONSOLE.info('Deleting ESX stack')
        return_code, stdout, stderr = self._terraform.destroy(capture_output=False)
        if return_code != 0:
    ...
    def _set_up_terraform_resources(self, node_counts):
        ...
        tf_work_dir = os.path.dirname('cli/tf-%s' % self._cluster)
        if not os.path.exists(tf_work_dir):
            os.makedirs(tf_work_dir)
        # Copy in deploy.tf and flavor/outputs.tf
        copyfile('terraform/deploy.tf', tf_work_dir)
        copyfile('terraform/%s/outputs.tf' % self._flavor, tf_work_dir)
        ...
        with open('%s/tfvars.tf' % tf_work_dir, 'wb') as vars_file:
